...
export class ManageOrderComponent implements OnInit {
  ...
  validateProductAdd() {
    if (this.manageOrderForm.controls['total'].value === 0 || this.manageOrderForm.controls['quantity'].value <= 0)
      return true;
  ...
  add() {
    ...
    if (productName === undefined) {
      ...
      this.dataSource.push({
        ...
        category: formData.category.name, quantity: formData.quantity, price: formData.price, total: formData.total
      });
  ...
  submitAction() {
    ...
    this.billService.generateReport(data).subscribe((response: any) => {
      this.dowloadFile(response?.uuid);
  ...
  dowloadFile(fileName: any) {
    var data = {
      uuid: fileName
    }
    this.billService.getPDF(data).subscribe((response: any) => {
      saveAs(response, fileName + '.pfd');
